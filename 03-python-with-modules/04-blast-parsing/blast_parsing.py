...
#                         if hit_match in alignment.hit_def:
#                             item = record.query, format_to_twenty_characters(alignment.hit_def), round(hsp.bits, 2), hsp.expect
#                             items.append(item)
#
#                         else:
#                             pass
#     if len(items) == 0:
#         sys.exit('warning: no hits')
#     else:
#         new_list = sorted(items, key=lambda x: x[-1])
#         print ('\n'.join(str(p) for p in new_list))
#
#
